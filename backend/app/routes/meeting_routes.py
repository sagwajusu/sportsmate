...
from app.models import Attendance, AttendanceCheckinWindow, ChatMessage, ChatRoom, Meeting, MeetingSession, Notice, Participant, Review, Sport, User, Vote, VoteOption, VoteResponse,MeetingView
...
from app.utils.attendance_policy import evaluate_host_manual_attendance, evaluate_qr_attendance
...
def resolve_attendance_session(meeting, requested_session_id=None):
    ...
    current_sessions = [
        ...
        if today <= item.start_at.date() < next_monday
    ]
    past_sessions = [
        item
        for item in reversed(sessions)
        if item.start_at.date() < today
    ]
...
def attendance_policy_response(result, default_status=409):
    ...
    return jsonify({
        ...
        "code": result.code,
    }), status_by_code.get(result.code, default_status)


def get_attendance_session_for_update(meeting_id, session_id):
    return (
        MeetingSession.query
...
def check_attendance(meeting_id):
    ...
    if meeting.host_id != current_user_id:
        return jsonify({
            "message": "방장만 참여자의 출석을 변경할 수 있습니다.",
            "code": "HOST_ONLY",
        }), 403
    try:
        target_user_id = int(data.get("user_id"))
    except (TypeError, ValueError):
        return jsonify({"message": "출석 대상 사용자를 확인해 주세요.", "code": "NOT_APPROVED"}), 400
    try:
    ...
    if not participant:
        return jsonify({
            "message": "승인된 참여자만 출석 처리할 수 있습니다.",
            "code": "NOT_APPROVED",
        }), 403
    policy = evaluate_host_manual_attendance(selected_session, kst_now())
    if not policy.allowed:
        return attendance_policy_response(policy)
...
def create_attendance_checkin_window(meeting_id):
    ...
    selected_session = get_attendance_session_for_update(meeting.id, selected_session.id)
    if not selected_session:
        return jsonify({"message": "출석 회차 정보를 확인할 수 없습니다.", "code": "INVALID_SESSION"}), 404
    policy = evaluate_qr_attendance(selected_session, kst_now(), meeting.status)
    if not policy.allowed:
        return attendance_policy_response(policy)
...
def attendance_qr_checkin(token):
    window = (
        ...
        .filter_by(token_hash=checkin_token_hash(token))
        .first()
    )
    if not window:
        return jsonify({
    ...
    session = get_attendance_session_for_update(
        window.meeting_session.meeting_id,
        window.meeting_session_id,
    )
    if not session:
        return jsonify({"message": "출석 회차 정보를 확인할 수 없습니다.", "code": "INVALID_SESSION"}), 404
    ...
    now = kst_now()
    policy = evaluate_qr_attendance(session, now, meeting.status)
    if not policy.allowed:
        if policy.code == "WINDOW_CLOSED" and window.is_active:
            window.is_active = False
            db.session.commit()
        return attendance_policy_response(policy)
    if not window.is_active:
        return jsonify({
            "message": "일정이 변경되었거나 새 QR이 발급되어 기존 QR을 사용할 수 없습니다.",
            "code": "QR_INVALIDATED",
        }), 409
    if now < window.opens_at:
        return jsonify({
            "message": "아직 출석 가능 시간이 아닙니다.",
            "code": "TOO_EARLY",
        }), 409
